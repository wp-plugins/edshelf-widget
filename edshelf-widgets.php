...
define( 'EDSHELF_DEFAULT_COLLECTION_HEIGHT', 550 );
define( 'EDSHELF_DEFAULT_COLLECTION_FORMAT', 'full' );
...
<p>The collection widget shortcode: <code>[edshelf-collection-widget id="NNNN" height="YYY" format="FFFF"]</code></p>
<p>The collection widget template tag: <code>edshelf_collection_widget( $id, $height, '$format' );</code></p>
...
<ol>
    ...
    <li>Look for the Widget module at the bottom of the right column. In that module will be some code that looks like this:<br><code>&lt;div id="edshelf-widget"&gt;&lt;/div&gt;&lt;script src="//edshelf.com/widgets/collection?id=8786&height=550&format=full"&gt;&lt;/script&gt;</code></li>
    <li>In that code, look for the ID number of the collection. In our example, it is:<br><code>8786</code>.</li>
    <li>In your WordPress site, go to the blog post or page on which you want to embed this collection widget. Type in the following WordPress shortcode in the text editor:<br><code>[edshelf-collection-widget id="NNNN" height="YYY" format="FFFF"]</code></li>
    ...
    <li>FFFF represents the format of widget you would like to display. There are three formats: "full", "compact", and "list". If you leave this out, the widget will automatically default to full. The differences between each are:<ul>
    ...
    <li>In our example, the final shortcode is:<br><code>[edshelf-collection-widget id="8786" height="800" format="full"]</code></li>
 ...
 */
function edshelf_collection_widget_embed( $id = EDSHELF_DEFAULT_COLLECTION_ID, $height = EDSHELF_DEFAULT_COLLECTION_HEIGHT, $format = EDSHELF_DEFAULT_COLLECTION_FORMAT ) {
    $id     = esc_attr( $id );
    $height = esc_attr( $height );
    $format = esc_attr( $format );
    return "<div id='edshelf-widget'></div><script src='//edshelf.com/widgets/collection?id={$id}&height={$height}&format={$format}&type=collection'></script>";
}
 ...
 */
function edshelf_collection_widget( $id, $height, $format ) {
    echo edshelf_collection_widget_embed( $id, $height, $format );
}
 ...
 *
 * [edshelf-collection-widget id="8786" height="500" format="full"]
 */
...
function edshelf_collection_widget_function( $atts ) {
    extract( shortcode_atts( array(
        ...
        'format' => EDSHELF_DEFAULT_COLLECTION_FORMAT
    ), $atts ) );

    return edshelf_collection_widget_embed( $id, $height, $format );
}
...
function edshelf_collection_widget_wpwidget( $args ) {
    ...
    echo $before_widget;
    echo edshelf_collection_widget_embed( $options['id'], $options['height'], $options['format'] );
    echo $after_widget;
}
...
function edshelf_collection_widget_control() {
    ...
    if( $_POST['edshelf_collection_submit'] ) {
        ...
        $options['format'] = esc_attr( $_POST['edshelf_collection_format'] );
...
<p>
    ...
    <label for="edshelf-collection-height">Widget format</label>:
    <input type="text" id="edshelf-collection-format" name="edshelf_collection_format" size="5" maxlength="10" value="<?php echo $options['format'];?>"><br>
