...
The shortcode is: `[edshelf-collection-widget id="NNNN" height="YYY"]`

The template tag is: `edshelf_collection_widget( NNNN, YYYY );`

Where `NNNN` is the ID of the collection you want to embed and `YYY` is the height of the widget in pixels. You can get this ID from the collection on edshelf, in the Widget module at the bottom of the right column.
...
1. Type in the following WordPress shortcode in the text editor: `[edshelf-collection-widget id="NNNN" height="YYY"]`, where `NNNN` is the collection ID you just found and `YYY` is the height of the widget, in pixels.
1. Or use the template tag `edshelf_collection_widget( $id, $height );` if you are familiar with PHP.
1. And you're done!
